...
function sessionSeconds(
  ...
  if (s.endedAt) return s.durationSec ?? Math.max(0, Math.round((s.endedAt.getTime() - s.startedAt.getTime()) / 1000));
  return Math.max(0, Math.round((now.getTime() - s.startedAt.getTime()) / 1000));
}

/** Online (working) seconds + the wall-clock span they cover. */
export function computeOnline(rows: OnlineRow[], now: Date): { onlineSec: number; spanSec: number } {
...
/**
 * The heuristic daily productivity score — same formula as GetProductivityUseCase:
 *   focus     = online − meeting
